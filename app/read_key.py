...
def read_and_decrypt_key(file_path, pin):
    encrypted_key, iv = read_private(file_path)
    private_key = decrypt_key(encrypted_key, iv, pin)
    print("Klucz prywatny został odszyfrowany.")
    return private_key


def read_public(file_path):
...
# klucz jest oczywiście wciąż zaszyfrowany ale można tak odczytać zaszyfrowany klucza i iv z pliku, żeby potem go 
# zdekryptować
def read_private(file_path):
